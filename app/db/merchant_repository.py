...
class MerchantRepository:
    ...
    def create(data: dict[str, Any]) -> int:
        ...
        cursor = conn.cursor()
        try:
            code = MerchantRepository._generate_code(cursor)
            cursor.execute(
                """INSERT INTO Merchant (code, name, contact_person, contact_phone,
                   contact_address, supplier_type, business_license, tax_number,
                   bank_name, bank_account, settlement_method, payment_terms,
                   credit_limit, level, status,
    ...
    def get_by_id(supplier_id: int) -> dict[str, Any] | None:
        return query_one("SELECT * FROM Merchant WHERE id = ?", (supplier_id,))

    @staticmethod
    def get_by_name(name: str) -> dict[str, Any] | None:
        return query_one(
            "SELECT * FROM Merchant WHERE name = ? AND status = 'active'", (name,)
        )
    ...
    def list(
        *,
        ...
        return query(
            f"SELECT * FROM Merchant WHERE {where} "
    ...
    def count(*, search: str = "", status: str | None = None, supplier_type: str | None = None, level: str | None = None) -> int:
        ...
        where = " AND ".join(clauses)
        row = query_one(f"SELECT COUNT(*) AS cnt FROM Merchant WHERE {where}", tuple(params))
    ...
    def update(supplier_id: int, data: dict[str, Any]) -> bool:
        ...
        if not data:
            return False
        for key in data:
            if key not in MerchantRepository._UPDATE_WHITELIST:
        ...
        try:
            cursor.execute(
                f"UPDATE Merchant SET {', '.join(sets)}, updated_at = CURRENT_TIMESTAMP "
                "WHERE id = ?",
                (*values, supplier_id),
            )
            conn.commit()
    ...
    def deactivate(supplier_id: int) -> bool:
        ...
        try:
            cursor.execute(
                "UPDATE Merchant SET status = 'inactive', updated_at = CURRENT_TIMESTAMP "
                "WHERE id = ?",
                (supplier_id,),
            )
            conn.commit()
    ...
    def hard_delete(supplier_id: int) -> bool:
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("DELETE FROM Merchant WHERE id = ?", (supplier_id,))
            conn.commit()
            return cursor.rowcount > 0
        except Exception:
            conn.rollback()
