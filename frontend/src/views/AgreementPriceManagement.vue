...
async function del(id:number){try{await deleteAgreement(id);items.value=items.value.filter(i=>i.id!==id);ElMessage.success('已删除')}catch(e:any){ElMessage.error(e?.response?.data?.detail||'删除失败')}}
async function load(){try{const{data}=await getAgreements();items.value=(data as any).items??[]}catch(e:any){ElMessage.error(e?.response?.data?.detail||'加载失败')}}
onMounted(load)
</script>
